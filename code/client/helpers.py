# =============================================================================
#  Copycord - Client UI Controller
#  Handles Admin UI -> Client commands over AdminBus
# =============================================================================
...
class ClientUiController:
    ...
    def start(self) -> None:
        if self._task and not self._task.done():
            self.log.debug("start() ignored; task already running: %s", self._task.get_name())
            return
        self._stopping = False
    ...
    async def stop(self) -> None:
        ...
        self._task.cancel()
        with asyncio.CancelledError.__enter__ if False else None:  # no-op, keeps linters calm
            pass
        try:
            await self._task
    ...
    async def _publish(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.log.debug("TX client event -> bus | ok req_id=%s took=%.1fms payload=%s",
                           payload.get("req_id"), self._ms_since(t0), _safe_preview(payload))
        except asyncio.CancelledError:
            raise
        except Exception as e:
            self.log.warning("TX client event -> bus FAILED | req_id=%s took=%.1fms err=%s payload=%s",
    ...
    async def _listen_loop(self):
        async def _handler(ev: dict):
            ...
            # Only handle UI -> client commands (avoid loops on our own ACKs: role='client')
            if kind != "client":
            ...
            # action/payload can be top-level or nested
            ...
            self.log.debug("Client command received | action=%r req_id=%s", action or "(none)", req_id)

            # Ignore non-command events silently (prevents publish→receive feedback loops)
            if not action:
            ...
            try:
                ...
                if action == "backfill":
                    self.log.debug("Backfill request received | req_id=%s raw=%s", req_id, _safe_preview(p))

                    # (1) Parse the id coming from UI (could be clone or original)
                    ui_raw = p.get("clone_channel_id")  # historical name; can be clone OR original
                    if ui_raw is None or str(ui_raw).strip() == "" or str(ui_raw).lower() == "null":
                        raise ValueError("channel_id missing")

                    any_id = int(ui_raw)

                    # (2) Resolve to original id regardless of what we got
                    orig_id, clone_id, src = self.listener.db.resolve_original_from_any_id(any_id)
                    if orig_id is None:
                        raise ValueError(f"Could not resolve original id from {any_id}")

                    self.log.debug(
                        "Backfill id resolved | src=%s ui_id=%s → original_id=%s clone_id=%s",
                        src, any_id, orig_id, clone_id
                    )
                    ...
                    rng = p.get("range") or {}
                    # after_iso: prefer flat field; fall back to range.after / range.value when mode=='since'
                    after_iso = p.get("after_iso")
                    if after_iso is None or (isinstance(after_iso, str) and after_iso.strip() == ""):
                        after_iso = rng.get("after")
                        if (after_iso is None or (isinstance(after_iso, str) and after_iso.strip() == "")) and \
                           (str(rng.get("mode") or "").lower() == "since"):
                            after_iso = rng.get("value")
                    if isinstance(after_iso, str) and after_iso.strip() == "":
                        after_iso = None

                    # last_n: prefer flat field; fall back to range.last_n / range.value when mode=='last'
                    ...
                    self.log.debug("Backfill params | mode=%s after_iso=%r last_n=%r", mode, after_iso, last_n)
                    ...
                    self.log.debug("ACK published to UI | original_id=%s clone_id=%s after_iso=%r last_n=%r",
                                  orig_id, clone_id, after_iso, last_n)
                    return

                # Unknown explicit action from UI
                self.log.debug("Unknown client action from UI | action=%r req_id=%s", action, req_id)
                await self._publish({
                    "kind": "client", "role": "client",
                    "payload": {"type": "error", "ok": False, "message": f"unknown action: {action}"}
                })

            except Exception as e:
                self.log.exception("Client command failed | req_id=%s action=%s", req_id, action)
                await self._publish({
                    "kind": "client", "role": "client",
                    "payload": {"type": "backfill_ack" if action == "backfill" else "error",
                                "ok": False, "error": str(e)}
                })
                self.log.debug("Error/NACK published | req_id=%s action=%s error=%s", req_id, action, e)
    ...
    async def _act_status(self, *, req_id: str, data: Dict[str, Any]):
        ...
        info = {
            ...
            "discord": {
                ...
                "user": str(getattr(self.bot, "user", None)) if getattr(self.bot, "user", None) else None,
